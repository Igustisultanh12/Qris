        ...
        <div v-if="selectedTx" class="space-y-6">
          ...
          <div class="flex flex-wrap justify-between items-center gap-2 pt-2">
            <div class="flex items-center gap-2">
              <button
                v-if="selectedTx.status === 'generated' || selectedTx.status === 'pending'"
                @click="simulatePaymentInModal(selectedTx.uuid || selectedTx.id)"
                :disabled="simulating"
                class="px-3.5 py-2 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-all shadow-md shadow-emerald-600/20 flex items-center gap-1.5 disabled:opacity-50"
              >
                <Loader2 v-if="simulating" class="w-3.5 h-3.5 animate-spin" />
                <CheckCircle2 v-else class="w-3.5 h-3.5" />
                <span>Simulasikan Lunas (PAID)</span>
              </button>

              <button
                v-if="selectedTx.status === 'generated'"
                @click="cancelTx(selectedTx.uuid || selectedTx.id)"
                class="px-3 py-2 text-xs font-bold text-rose-600 bg-rose-50 dark:bg-rose-950/40 rounded-xl hover:bg-rose-100"
              >
                Batalkan
              </button>
            </div>
...
const simulatePaymentInModal = async (id: any) => {
  simulating.value = true;
  try {
    await api.post(`/customer/transactions/${id}/simulate-paid`);
    if (selectedTx.value) {
      selectedTx.value.status = 'paid';
    }
    toast.success('Pembayaran Dikonfirmasi!', 'Status transaksi kini LUNAS (PAID) dan notifikasi webhook berhasil dikirim.');
    fetchTransactions(meta.value?.current_page || 1);
  } catch (err: any) {
